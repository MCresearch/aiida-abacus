...
We provide an example configuration file in [examples](https://github.com/MCresearch/aiida-abacus/tree/develop/examples) directory. 

```bash
cd examples
# remote-slurm-ssh-setup.yml
verdi computer setup --config remote-slurm-ssh-setup.yml
```
You can freely edit the configuration [YAML](https://yaml.org/) file or set up a new computer from scratch by:
```console
verdi computer setup
```
If ABACUS is built with [the Intel® oneAPI](https://www.intel.cn/content/www/cn/zh/developer/tools/oneapi/base-toolkit-download.html), please change the  `prepend_text` to set up your oneAPI environment, where you can include `bash` commands that will be executed before running the submission script. Just remove the line in the configuration file if not needed. Consult [AiiDA computer-setup](https://aiida.readthedocs.io/projects/aiida-core/en/latest/howto/run_codes.html#computer-setup) for a complete guide.

If you want to do the calculations on the local machine with AiiDA environment, please change to `core.local` by modifying the configuration YAML file.
